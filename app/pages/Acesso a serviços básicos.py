...
@st.cache_data
def get_valores_possiveis(coluna: str) -> list[str]:
    ...
    match servico:
        case "Abastecimento de água":
            query = f"""
                ...
                from microdados
                group by NU_ANO_CENSO, {dimensao_geografica}, {dimensao}
                order by 1, 2, 3
            """
        ...
        case "Abastecimento de água":
            query = f"""
                select
                    ...
                    , 3) as 'Sem informações'
                from microdados
                where {dimensao}='{filtro_dimensao}'
...
def linha(filtro_dimensao: str, label_dimensao: str, label_dimensao_geografica: str, servico: str) -> None:
    filtro_dimensao_geografica = st.sidebar.selectbox(
        ...
        get_valores_possiveis(DIMENSOES[label_dimensao_geografica])
    )
    df = get_df_linha(
        servico,
        DIMENSOES[label_dimensao_geografica],
        label_dimensao_geografica,
...
def main() -> None:
    ...
    filtro_dimensao = st.sidebar.selectbox(
        label_dimensao,
        get_valores_possiveis(DIMENSOES[label_dimensao])
    )
